...
void Factory::printFactory(){
    ...
    for(unsigned int i = 0; i< tiles->size(); ++i){
        ...
        if(colour == R) {
            std::cout << BOLDRED <<"R"<< COLOURENDING <<" ";
        } else if(colour == Y) {
            std::cout << BOLDYELLOW <<"Y"<< COLOURENDING <<" ";
        } else if(colour == B) {
            std::cout << BOLDBLUE <<"B"<< COLOURENDING <<" ";
        } else if(colour == L) {
            std::cout << BOLDCYAN <<"L"<< COLOURENDING <<" ";
        }else if(colour == U) {
            std::cout << BOLDBLACK <<"U"<< COLOURENDING <<" ";
        } else if(colour == F) {
            std::cout << 'F' << " ";
        }
    }
...
int Factory::findColourOfGreatestAmountTile(){
    int count = 0;
    int colourWithGreatestCount = 0;

    for(int c = 1; c <= NUMCOLOURS; ++c){
        if(getNumberTilesOfColour(c) > count){
            count = getNumberTilesOfColour(c);
            colourWithGreatestCount = c;
        }
    }

    return colourWithGreatestCount;
}

int Factory::findColourOfLeastAmountTile(){
    int count = 6;
    int colourWithSmallestCount = 0;

    for(int c = 1; c <= NUMCOLOURS; ++c){
        if(getNumberTilesOfColour(c) < count){
            count = getNumberTilesOfColour(c);
            colourWithSmallestCount = c;
        }
    }

    return colourWithSmallestCount;
}

void Factory::setCentre(bool centre){
    this->centre = centre;
}
